...
export default function Navbar() {
  ...
  return (
    <header className="w-full sticky top-0 z-50 h-20 bg-white border-b border-gray-100 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex items-center justify-between">
        ...
        >
          <div className="flex items-center gap-1 select-none">
            <span className="relative flex items-center">
              <span className="font-extrabold text-[#d97706] tracking-tight font-serif italic text-3xl sm:text-4xl">
                haldimehendi
              </span>
              
              {/* Interlocking Rings Emblem over 'aa' */}
              <span className="absolute left-7 sm:left-9 flex items-center -space-x-1 -top-1.5 sm:-top-2">
                <span className="rounded-full border-2 border-[#d97706] bg-transparent w-3 h-3 sm:w-3.5 sm:h-3.5"></span>
                <span className="rounded-full border-2 border-[#15803d] bg-transparent w-3 h-3 sm:w-3.5 sm:h-3.5"></span>
              </span>
            </span>

            {/* .com Tag */}
            <span className="font-bold text-[#15803d] self-end text-sm sm:text-base mb-1.5 ml-1">
              .com
            </span>
          </div>
        </Link>
        ...
        <div className="hidden md:flex items-center space-x-8 lg:space-x-12">
          <Link
            href="/about"
            className="font-semibold text-gray-700 hover:text-[#d97706] transition-colors py-1 text-base lg:text-lg"
          >
            <span>About us</span>
          </Link>
          ...
          <div className="relative" ref={dropdownRef}>
            <button
              ...
              className={`rounded-full border font-semibold px-5 py-2 text-base transition-colors flex items-center gap-2 cursor-pointer outline-hidden ${
                isDropdownOpen
            ...
            >
              <span>Login</span>
              <svg
                className={`w-4 h-4 transition-transform duration-200 ${isDropdownOpen ? 'rotate-180 text-[#d97706]' : 'text-gray-500'}`}
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                strokeWidth={2.5}
            ...
            {/* Dropdown Menu */}
            {isDropdownOpen && (
              <div className="absolute right-0 mt-3 w-72 bg-white rounded-2xl shadow-xl border border-gray-100 py-3 z-50 origin-top-right">
        ...
        <div className="flex md:hidden items-center space-x-3">
          <button
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            className="font-semibold rounded-full border border-gray-300 text-gray-700 px-4 py-1.5 text-sm flex items-center gap-1 cursor-pointer hover:border-[#d97706] hover:text-[#d97706]"
          >
            <span>Login</span>
            <svg
